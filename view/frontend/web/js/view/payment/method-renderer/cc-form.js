...
define(
    [
        ...
        'TheVaultApp_Magento2/js/view/payment/response-strategy',
        'Magento_Checkout/js/checkout-data',
        'Magento_Checkout/js/model/payment/additional-validators'
    ],
    function ($, Component, VaultEnabler, TheVaultApp, quote, placeOrderAction, responseStrategy, checkoutData, additionalValidators) {
        'use strict';

        return Component.extend({
            defaults: {
                active: true,
                template: 'TheVaultApp_Magento2/payment/form',
                code: 'thevaultapp',
                card_token_id: null,
                creditCardHolder: ''
            },
            ...
            beforePlaceOrder: function() {
                if (additionalValidators.validate()) {               
                    var self = this;
                    TheVaultApp.getClient().done(function() {
                        CheckoutKit.createCardToken(self.getCardTokenData(), {includeBinData: false}, function(response) {
                            if ('error' === response.type) {
                                TheVaultApp.showError('It looks like you have entered incorrect bank card data.');
                            }
                            else {
